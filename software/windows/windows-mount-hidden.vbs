...
' 1. Primary Attempt: Spawn completely hidden background process (ShowWindow = 0)
cmdMount = Q & rcloneBin & Q & " mount " & remotePath & " " & driveLetter & " --vfs-cache-mode full --vfs-cache-max-size 10G --vfs-cache-max-age 24h --vfs-write-back 1s --dir-cache-time 10s --attr-timeout 1s --contimeout 60s --timeout 60s --low-level-retries 10 --retries 10 --gcs-bucket-policy-only --rc --rc-no-auth --rc-addr 127.0.0.1:5572 --no-modtime --log-level NOTICE " & readOnlyFlag
